...
class LoginSpec extends Specification with GWT with StandardDelimitedStepParsers { def is = s2"""
  ...
  正常にログインできること
    Given ログインID {taro@LoginSpec}、パスワード {pass}、名前 {タスク太郎} のユーザが存在する  $g1
    When ログインID {taro@LoginSpec}、パスワード {pass} でログインする  $w1
  ...
  def g1 = step(threeStrings) { _ match { case (loginId, password, userName) =>
    usersApi.registerUser(loginId, password, userName)
  }} 
